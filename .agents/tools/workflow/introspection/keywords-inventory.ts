...
function parseArgs(): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--last") args.last = Number(argv[++i]);
      ...
      console.log(`keywords-inventory — keyword counts across reflections

Usage: keywords-inventory.ts [options]
...
async function main() {
  ...
  for (const ns of namespaces) {
    const recent = await client.query(api.reflections.recent, {
      password: config.password,
      namespaceId: ns._id,
      last: args.last,
    });
