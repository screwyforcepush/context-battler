...
## 2a. HTTP replay contract (render R&D round-3)
...
| `/replay/exportMatch?matchId=<id>` | GET | `MatchSnapshotJson` (schemaVersion 2) — full match snapshot | 400 (missing/malformed id), 404 (not found), 409 (not completed) |
...
curl -sS "$CONVEX_URL/replay/listMatches" | jq '.[0]'

# Export a specific match:
MATCH_ID=$(curl -sS "$CONVEX_URL/replay/listMatches" | jq -r '.[0].matchId')
curl -sS "$CONVEX_URL/replay/exportMatch?matchId=$MATCH_ID" | jq '.schemaVersion, .playback.turnCount'
```

Types: `MatchSnapshotJson` and `MatchSummary` are exported from `convex/replay/snapshotTypes.ts`. Full contract shape documented in [`docs/project/phases/render-rnd/round-3-closing-readout.md` §3](../docs/project/phases/render-rnd/round-3-closing-readout.md).
