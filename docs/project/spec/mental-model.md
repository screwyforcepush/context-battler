...
## 11. Current vision — equipment variance & contested public objectives
...
- **When should the agent choose *what* to loot?** Today a loot pulls a fixed set. The near-future direction is letting the agent pick items from any lootable entity — but that is a no-brainer ("always take the upgrade") unless equipment carries *varied* stats beyond raw damage, so the choice has real trade-offs (range vs. damage, weight, the consumable opportunity cost). New tool-schema surface, sequenced *after* equipment variance lands; a likely-soon direction, not yet scoped.
