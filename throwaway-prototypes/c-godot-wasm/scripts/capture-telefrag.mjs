...
Captures telefrag-capture.png and telefrag-capture.gif from the Godot WASM
prototype once the export exists. By default it serves built dist/ with gzip.
...
async function launchChrome(opts) {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "c-godot-wasm-capture-"));
...
async function main() {
  ...
  const frameDir = mkdtempSync(path.join(os.tmpdir(), "c-godot-wasm-frames-"));
  let chrome = null;
  let cdp = null;
  const frames = [];
