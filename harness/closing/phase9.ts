...
import type { PersonaId, Tile, Wall } from "../../convex/engine/types.js";
import type { Phase7RunStatsRow } from "../../convex/reports/phase7.js";
import {
...
export type Phase9ClosingResult = {
  exitCode: number;
  reportId?: string;
  existed?: boolean;
  payload?: Phase9Payload;
...
const worldStateByMatchId = makeFunctionReference<
  "query",
  { matchId: MatchId },
  WorldStateRow
>("worldState:byMatchId");

const persistComputedPhase9Report = makeFunctionReference<
  "mutation",
  { matchIds: MatchId[]; payload: Phase9Payload; overwrite?: boolean },
  PersistResult
>("reports/phase9:persistComputedPhase9Report");

export function parsePhase9ClosingArgs(argv: readonly string[]): Phase9ClosingArgs {
...
export async function runPhase9ClosingCli(
  argv = process.argv.slice(2),
  deps: Phase9ClosingDeps = {},
): Promise<Phase9ClosingResult> {
  const args = parsePhase9ClosingArgs(argv);
  if (args.help) {
    const usage = [
      "usage: npx tsx harness/closing/phase9.ts [--last N | --matchIds a,b] [--overwrite]",
      "",
      "Explicit positional match ids are also accepted:",
      "  npx tsx harness/closing/phase9.ts <matchId1> <matchId2>",
      "",
    ].join("\n");
  ...
  if (matchIds.length === 0) {
    throw new Error("No match ids resolved for phase-9 closing report");
  }
  if (matchIds.length > MAX_MATCHES) {
    throw new Error(`Phase 9 closing accepts at most ${MAX_MATCHES} matches`);
  }

  const runs = await buildPhase9RunInputs(client, matchIds, deps);
  const payload = computePhase9Metrics(runs);
  const persisted = await (deps.persistReport ?? persistPhase9Report)(client, {
    matchIds,
    payload,
...
async function buildPhase9RunInputs(
  client: ClosingClient,
  matchIds: MatchId[],
  deps: Phase9ClosingDeps,
): Promise<Phase9RunInput[]> {
  const [slimRows, runRows, statuses, worldRows] = await Promise.all([
    (deps.fetchSlimAcross ?? fetchPhase9SlimAcross)(client, matchIds),
    Promise.all(
    ...
    Promise.all(
      matchIds.map((matchId) =>
        (deps.readWorldStateByMatch ?? readWorldStateByMatch)(
          client,
          matchId,
        ),
      ),
  ...
  return matchIds.map((matchId, index) => {
    ...
    const worldState = adaptWorldState(worldRows[index], matchId);
...
async function fetchPhase9SlimAcross(
  client: ClosingClient,
  matchIds: MatchId[],
): Promise<SlimMatchRows[]> {
  try {
...
async function readWorldStateByMatch(
  client: ClosingClient,
  matchId: MatchId,
): Promise<WorldStateRow> {
  return (await client.query(worldStateByMatchId, { matchId })) as WorldStateRow;
}

async function persistPhase9Report(
  client: ClosingClient,
  args: {
    matchIds: MatchId[];
    payload: Phase9Payload;
  ...
  return (await client.mutation(persistComputedPhase9Report, {
...
function adaptRunSummary(row: RunSummaryRow | undefined): Phase7RunStatsRow | null {
...
function adaptWorldState(
  row: WorldStateRow | undefined,
  matchId: MatchId,
...
if (isMain()) {
  try {
    const result = await runPhase9ClosingCli();
