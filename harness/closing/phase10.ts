...
export type Phase10ClosingArgs = {
  last: number;
  matchIds: MatchId[];
  overwrite: boolean;
  help: boolean;
...
export type Phase10ClosingDeps = {
...
export type Phase10ClosingResult = {
  exitCode: number;
  reportId?: string;
  existed?: boolean;
  payload?: Phase10Payload;
...
const persistComputedPhase10Report = makeFunctionReference<
  "mutation",
  { matchIds: MatchId[]; payload: Phase10Payload; overwrite?: boolean },
  PersistResult
>("reports/phase10:persistComputedPhase10Report");

export function parsePhase10ClosingArgs(argv: readonly string[]): Phase10ClosingArgs {
...
export async function runPhase10ClosingCli(
  argv = process.argv.slice(2),
  deps: Phase10ClosingDeps = {},
): Promise<Phase10ClosingResult> {
  const args = parsePhase10ClosingArgs(argv);
  if (args.help) {
    const usage = [
      "usage: npx tsx harness/closing/phase10.ts [--last N | --matchIds a,b] [--overwrite]",
      "",
      "Explicit positional match ids are also accepted:",
      "  npx tsx harness/closing/phase10.ts <matchId1> <matchId2>",
      "",
    ].join("\n");
  ...
  if (matchIds.length === 0) {
    throw new Error("No match ids resolved for phase-10 closing report");
  }
  if (matchIds.length > MAX_MATCHES) {
    throw new Error(`Phase 10 closing accepts at most ${MAX_MATCHES} matches`);
  }

  const runs = await buildPhase10RunInputs(client, matchIds, deps);
  const payload = computePhase10Metrics(runs);
  const persisted = await (deps.persistReport ?? persistPhase10Report)(client, {
    matchIds,
    payload,
...
async function buildPhase10RunInputs(
  client: ClosingClient,
  matchIds: MatchId[],
  deps: Phase10ClosingDeps,
): Promise<Phase10RunInput[]> {
  const [slimRows, runRows, statuses] = await Promise.all([
    (deps.fetchSlimAcross ?? fetchPhase10SlimAcross)(client, matchIds),
    Promise.all(
...
async function fetchPhase10SlimAcross(
  client: ClosingClient,
  matchIds: MatchId[],
): Promise<SlimMatchRows[]> {
  try {
...
async function persistPhase10Report(
  client: ClosingClient,
  args: {
    matchIds: MatchId[];
    payload: Phase10Payload;
  ...
  return (await client.mutation(persistComputedPhase10Report, {
...
if (isMain()) {
  try {
    const result = await runPhase10ClosingCli();
