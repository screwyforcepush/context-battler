...
export type Phase7ClosingResult = {
  exitCode: number;
  reportId?: string;
  existed?: boolean;
  payload?: Phase7Payload;
...
const persistComputedPhase7Report = makeFunctionReference<
  "mutation",
  { matchIds: MatchId[]; payload: Phase7Payload; overwrite?: boolean },
  PersistResult
>("reports/phase7:persistComputedPhase7Report");

export function parsePhase7ClosingArgs(argv: readonly string[]): Phase7ClosingArgs {
...
export async function runPhase7ClosingCli(
  argv = process.argv.slice(2),
  deps: Phase7ClosingDeps = {},
): Promise<Phase7ClosingResult> {
  const args = parsePhase7ClosingArgs(argv);
  if (args.help) {
    const usage = [
      "usage: npx tsx harness/closing/phase7.ts [--last N | --matchIds a,b] [--overwrite]",
      "",
      "Explicit positional match ids are also accepted:",
      "  npx tsx harness/closing/phase7.ts <matchId1> <matchId2>",
      "",
    ].join("\n");
  ...
  if (matchIds.length === 0) {
    throw new Error("No match ids resolved for phase-7 closing report");
  }
  if (matchIds.length > MAX_MATCHES) {
    throw new Error(`Phase 7 closing accepts at most ${MAX_MATCHES} matches`);
  }

  const runs = await buildPhase7RunInputs(client, matchIds, deps);
  const payload = computePhase7Metrics(runs);
  const persisted = await (deps.persistReport ?? persistPhase7Report)(client, {
    matchIds,
    payload,
...
async function buildPhase7RunInputs(
  client: ClosingClient,
  matchIds: MatchId[],
  deps: Phase7ClosingDeps,
): Promise<Phase7RunInput[]> {
  const [slimRows, runRows, statuses] = await Promise.all([
    (deps.fetchSlimAcross ?? fetchPhase7SlimAcross)(client, matchIds),
    Promise.all(
...
async function fetchPhase7SlimAcross(
  client: ClosingClient,
  matchIds: MatchId[],
): Promise<SlimMatchRows[]> {
  try {
...
async function persistPhase7Report(
  client: ClosingClient,
  args: {
    matchIds: MatchId[];
    payload: Phase7Payload;
  ...
  return (await client.mutation(persistComputedPhase7Report, {
...
if (isMain()) {
  try {
    const result = await runPhase7ClosingCli();
